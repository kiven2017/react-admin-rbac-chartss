...
export default function P13() {
  ...
  const fields=[{"field": "id", "label": "id", "type": "uuid", "required": true}, {"field": "name", "label": "name", "type": "string", "required": true}, {"field": "status", "label": "status", "type": "enum", "required": false}, {"field": "description", "label": "description", "type": "text", "required": false}, {"field": "created_at", "label": "created_at", "type": "datetime", "required": false}, {"field": "updated_at", "label": "updated_at", "type": "datetime", "required": false}, {"field": "device_id", "label": "device_id", "type": "string", "required": true}, {"field": "serial_number", "label": "serial_number", "type": "string", "required": true}, {"field": "did", "label": "did", "type": "string", "required": true}, {"field": "owner_address", "label": "owner_address", "type": "string", "required": false}, {"field": "firmware_version", "label": "firmware_version", "type": "string", "required": false}, {"field": "lifecycle_status", "label": "lifecycle_status", "type": "enum", "required": false}, {"field": "activated_at", "label": "activated_at", "type": "datetime", "required": false}, {"field": "last_seen_at", "label": "last_seen_at", "type": "datetime", "required": false}, {"field": "latency_ms", "label": "latency_ms", "type": "number", "required": false}, {"field": "cpu_usage", "label": "cpu_usage", "type": "number", "required": false}, {"field": "mem_usage", "label": "mem_usage", "type": "number", "required": false}, {"field": "block_height", "label": "block_height", "type": "number", "required": false}, {"field": "peer_count", "label": "peer_count", "type": "number", "required": false}]
  const onSubmit=(payload)=>createByFeature(featureSlug,payload).then(fetchData)
  return (
    <div className='space-y-4'>
      <div className='flex items-center justify-between'>
        <div><h1 className='text-2xl font-semibold'>节点数据管理</h1><p className='text-muted text-sm'>提供数据浏览器功能，允许按设备 DID 或数据源查看原始和已处理的数据，便于调试和验证数据准确性 。</p></div>
