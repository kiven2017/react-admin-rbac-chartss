...
export default function Login() {
  ...
  async function submit(e) {
    ...
    try {
      ...
      const token = data?.token || manualToken
      const roleFromApi = data?.role || role
      value.login(email, roleFromApi, token)
      nav(from, { replace: true })
    } catch (err) {
      if (manualToken) {
        value.login(email, role, manualToken)
        nav(from, { replace: true })
      } else {
    ...
    <div className="min-h-screen grid place-items-center">
      <form onSubmit={submit} className="card p-8 w-[360px] space-y-4">
        <div className="text-center space-y-2">
          ...
          <p className="text-muted text-sm">请选择一个角色体验 RBAC</p>
        </div>
        ...
        <div className="space-y-1">
          <label className="text-sm text-muted">角色</label>
...
<div className="space-y-1">
  <label className="text-sm text-muted">若登录接口不可用，可手动粘贴 Token</label>
  <input className="input w-full" value={manualToken} onChange={e=>setManualToken(e.target.value)} placeholder="Bearer Token" />
</div>
