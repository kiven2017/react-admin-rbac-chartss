...
export default function 1_链上状态监控() {
  const featureSlug = "链上状态监控"
    ...
    <ErrorBoundary>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold">链上状态监控</h1>
            <p className="text-muted text-sm">监控并展示数据合约的最新高度、预言机数据源的新鲜度（可使用绿/黄/红状态灯表示）、以及当前签名节点集合 。当数据延迟超过阈值时，系统应发出警报 。</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <PermissionGate perm="链上状态监控:export"><button className="btn">导出</button></PermissionGate>
            <PermissionGate perm="链上状态监控:create"><button className="btn btn-primary" onClick={()=>setFormOpen(v=>!v)}>新增</button></PermissionGate>
          </div>
        </div>
