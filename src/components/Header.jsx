...
export default function Header() {
  const { value } = useAuth()
  const { lang, switchTo } = useI18n()
  return (
    <header className="sticky top-0 z-10 bg-bg/90 backdrop-blur border-b border-border">
      <div className="max-w-[1400px] mx-auto px-6 h-14 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-6 h-6 rounded-full" style={{background:'linear-gradient(135deg,#C6A15B,#E0C27A)'}}></div>
          <div className="font-semibold">AI Oracle Admin</div>
        </div>
