...
export function DynamicForm({ fields, onSubmit }) {
  ...
  function validate() {
    const e = {}
    fields.forEach(f => {
      if (f.required && !data[f.field]) e[f.field] = '必填'
      if (f.type === 'number' && data[f.field] && isNaN(Number(data[f.field]))) e[f.field] = '必须为数字'
      // 简单示例，可根据需要扩展
    })
    setErrors(e)
  ...
  return (
    <form onSubmit={submit} className="card p-4 space-y-4 shadow-soft">
...
function renderInput(f, value, onChange) {
  switch (f.type) {
    ...
    case 'uuid':
    case 'json':
      return <input className="input" value={value} onChange={e=>onChange(e.target.value)} placeholder={f.field} />
    case 'number':
      return <input className="input" value={value} onChange={e=>onChange(e.target.value)} placeholder="数字" />
    case 'date':
